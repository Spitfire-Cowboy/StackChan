...
- [WebSocket Display Cards](#websocket-display-cards)
...
## WebSocket Display Cards

The StackChan websocket binary protocol already uses `TextMessage` packets (`0x07`) for remote speech messages.
This repo now also supports a structured `displayCard` payload on that same transport for temporary on-device status cards in the avatar app.

### Packet Framing

Binary websocket packets are framed as:

```text
[type:1 byte][length:4 bytes big-endian][payload:length bytes]
```

For text messages and display cards, `type` is `0x07`.

### Payload Shapes

Legacy speech payload:

```json
{
  "name": "Operator",
  "content": "The equipment has been reconnected."
}
```
...
```json
{
  "type": "displayCard",
  "title": "SYSTEM NOTICE",
  "lines": ["Wi-Fi Connected", "Battery 82%", "Camera Ready", "Tap to Continue"],
  "accent": "#FFDF9A",
  "timeoutMs": 4000
}
```

Sticky display card payload:

```json
{
  "type": "displayCard",
  "title": "ACTION REQUIRED",
  "lines": ["Tap to Continue"],
  "accent": "#FFD36A",
  "sticky": true
}
```

Clear display card payload:
...
Notes:
- `content` still works for legacy speech messages.
- `displayCard` accepts `content` as a single-line fallback if `lines` is omitted on the firmware side.
- `accent` should be a 6-digit hex color string like `#FFDF9A`.
- `sticky: true` keeps the card visible until it is explicitly cleared.
- `clear: true` dismisses the current display card without requiring replacement lines.

### Go Helper Package

Use the typed helpers in `server/internal/wsproto` instead of hand-building the JSON and binary framing:

```go
packet, err := wsproto.CreateDisplayCardPacket(
    "SYSTEM NOTICE",
    []string{"Wi-Fi Connected", "Battery 82%", "Camera Ready", "Tap to Continue"},
    "#FFDF9A",
    4000,
    false,
    false,
...
web_socket.SendStackChanMessage(ctx, mac, &messageType, &packet, nil)
```

If you are already in the websocket server layer, use the convenience wrappers in `server/internal/web_socket`:

```go
if err := web_socket.SendDisplayCard(
    ctx,
    mac,
    "SYSTEM NOTICE",
    []string{"Wi-Fi Connected", "Battery 82%", "Camera Ready", "Tap to Continue"},
...
To post a sticky card:

```go
err := web_socket.SendDisplayCard(
    ctx,
    mac,
    "ACTION REQUIRED",
...
To clear the active card:

```go
err := web_socket.SendDisplayCard(ctx, mac, "", nil, "", 0, false, true)
```

For legacy speech bubbles:

```go
packet, err := wsproto.CreateTextMessagePacket("Operator", "The equipment has been reconnected.")
```

### HTTP Device Endpoint

If you want to trigger a display card through the authenticated device API instead of calling the websocket helpers directly, use:

```text
POST /stackChan/v2/device/displayCard
...
```bash
curl -X POST http://127.0.0.1:12800/stackChan/v2/device/displayCard \
...
- the device is bound to the authenticated user
- the StackChan is currently connected to the websocket server
- at least one non-empty display card line is provided unless `clear` is true
---
